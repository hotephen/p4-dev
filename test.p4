...
action set_stage_and_bitmap(action_bitmap, match_bitmap, next_stage, next_prog) {
    ...
    action set_action_id( match_result, action_bitmap, match_bitmap, next_stage, next_prog) {
    ...
    table table_mod_std_meta_stage1 {
        reads {
            vdp_metadata.action_chain_id: exact;
        }
        actions {
            do_forward;
            do_drop;
...
control ingress{
    ...
    if ((vdp_metadata.stage_id & CONST_NUM_OF_STAGE) == 1) {
      ...
      apply(table_header_match_stage1_2);
        ...
        if (vdp_metadata.action_chain_bitmap != 0) {
            if ((vdp_metadata.action_chain_bitmap & BIT_MASK_MOD_STD_META) != 0) {
