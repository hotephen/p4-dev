...
Register<bit<32>,bit<16>>(1024,0)  set_pkt_id_reg;

RegisterAction<bit<32>, bit<16>, bit<32>>(set_pkt_id_reg) read = {
    void apply(inout bit<32> register_data, out bit<32> result) {
	result = register_data;
	register_data = meta.pkt_id.id + 1;
	
    }
};

/*
RegisterAction<bit<32>, bit<32>, bit<32>>(set_pkt_id_reg) write = {
...
RegisterAction<bit<32>, bit<16>, bit<32>>(set_pkt_id_reg) write_0 = {
    void apply(inout bit<32> register_data) {
        register_data = 0;
    }
};
...
//SF3 actions
	...
	meta.ecmp_select = hash.get({hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort}, ecmp_base, ecmp_count);	
