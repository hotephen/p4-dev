...
local SPI		= 1
local SI 		= 255
local ETH_DST	= "00:00:00:00:00:02"
...
function configure(parser)
	...
	parser:option("-n --filename", "Filename to save latency."):default("processing_latency")
	parser:option("-h --histogram", "Filename of the latency histogram."):default("histogram.csv")
end
...
function master(args)
	...
	mg.startSharedTask("timerSlave", txDev:getTxQueue(1), rxDev:getRxQueue(1), args.histogram)
...
function timerSlave(txQueue, rxQueue, histfile)
	...
	while mg.running() do
		hist:update(timestamper:measureLatency(function(buf) buf:getNshPacket().innerEth.dst:setString(ETH_DST) end))
	end
	hist:print()
	hist:save(histfile)
end