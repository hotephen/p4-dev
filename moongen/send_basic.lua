...
function configure(parser)
	...
	parser:option("-receive --receive", "Receive."):default(0):convert(tonumber)
...
function master(args)
	...
	mg.startTask("loadSlave", txDev:getTxQueue(0), rxDev, args.size, args.flows, args.txdump)
...
function loadSlave(queue, rxDev, size, flows, txdump)
	-- doArp()
	...
	local dstIP = parseIPAddress("10.0.2.1")
	while mg.running() do
		bufs:alloc(size)
