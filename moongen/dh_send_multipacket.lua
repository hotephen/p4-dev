...
function loadSlave(queue, rxDev, size, flows, dh, txdump)
	...
	local srcIP = parseIPAddress("10.10.0.1")
	local dstIPl2fwd = parseIPAddress("10.10.0.5")
	local dstIPl3fwd = parseIPAddress("10.10.0.6")
	local dstIPfw = parseIPAddress("10.10.0.7")
	local dstIPlb = parseIPAddress("10.10.0.8")
	...
	while mg.running() do
		...
		for i, buf in ipairs(bufs) do
			...
			pkt.dh.len = 0x3
			if i%4 == 0 then
				pkt.eth.dst:set(0x050000000000) --"00:00:00:00:00:05"
				pkt.eth.src:set(0x010000000000) --"00:00:00:00:00:01"
				pkt.ip4.src:set(srcIP)
				pkt.ip4.dst:set(dstIPl2fwd)
				pkt.dh.vdp_id = 0x0100 -- 0001
				queue:send(bufs)
			elseif i%4 == 1 then
				pkt.eth.dst:set(0x060000000000) --"00:00:00:00:00:06"
				pkt.eth.src:set(0x010000000000) --"00:00:00:00:00:01"
				pkt.ip4.src:set(srcIP)
				...
				queue:send(bufs)
			elseif i%4 == 2 then
				pkt.eth.dst:set(0x070000000000) --"00:00:00:00:00:07"
				pkt.eth.src:set(0x010000000000) --"00:00:00:00:00:01"
				pkt.ip4.src:set(srcIP)
				...
				queue:send(bufs)
			elseif i%4 == 3 then
				pkt.eth.dst:set(0x080000000000) --"00:00:00:00:00:08"
				pkt.eth.src:set(0x010000000000) --"00:00:00:00:00:01"
				pkt.ip4.src:set(srcIP)
				pkt.ip4.dst:set(dstIPlb)
				pkt.tcp:setSrc(1234)
				pkt.tcp:setDst(80)
				pkt.dh.vdp_id = 0x0400 -- 0004
				queue:send(bufs)
			end
