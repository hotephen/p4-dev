    ...
    apply{
        ...
        if(meta.frame_type == 1){ // When packet is local gradient
            ...
            if(hdr.preamble.pool_version == 1){ // When version (pool) is 1 
                meta.worker_bitmap_mask = (bit<32>)0x00000001 >> hdr.preamble.worker_id;
                ...
                seen_pool1.read(seen_pool1_bitmap, meta.pool_index);

                // if(sent_seg_num == 1){
                if(meta.worker_bitmap_mask & seen_pool1_bitmap != 0)
                    // No Actions, because packet is retransmission
                    drop();
                }
                else{ // before K
                    seen_pool1_bitmap = seen_pool1_bitmap ^ meta.worker_bitmap_mask;
                    seen_pool1.write(meta.pool_index, seen_pool1_bitmap);
            ...
            /* --------------------- pool2 --------------------- */
                ...
                seen_pool2.read(seen_pool2_bitmap, meta.pool_index);

                // if(sent_seg_num == 1){
                if(meta.worker_bitmap_mask & seen_pool2_bitmap != 0)
                    ...
                    if(meta.counter_value >= meta.k){    // When all gradients are received : broadcast
                        ...
                        // // Check that the sum of the signs is greater than threshold
                        // if(meta.end == 1){
                        //     if (meta.sum_grad_sign >= 400000){  // s_threshold
                        //         // Increase count
                        //         counter.read(meta.counter, 0);
                        //         meta.counter = meta.counter + 1;
                        //         counter.write(0, meta.counter);

                        //         if (meta.counter >= 10){        // c_threshold
                        //             // Increase k 
                        //             k.read(meta.k, 0)
                        //             meta.k = meta.k + 1;
                        //             k.write(0,meta.k)
