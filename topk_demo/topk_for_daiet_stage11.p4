...
#define NUMBER_OF_CELLS 16w136 // to be modifed for adjusting register size
// ----------------------------
// ---------- header ----------
// ----------------------------
...
struct metadata {
    ...
    bit<32> tree_id_for_hash10;
    bit<32> tree_id_for_hash11;
    bit<32> remaining_number_of_entries;
    bit<8>  flush;
        ...
        if ( hdr.frame_type.frame_type == 0 ) {
            ...
            { \ 
                if(hdr.preamble.remaining_number_of_entries != 0){ \
                    ...
                    pushout_key=0; \
                    pushout_value=0; \
                    hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id-2)*NUMBER_OF_CELLS, { hdr.entry[##i].key }, NUMBER_OF_CELLS); \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32_custom, ((bit<16>)meta.tree_id-1)*NUMBER_OF_CELLS+NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc16, ((bit<16>)meta.tree_id)*NUMBER_OF_CELLS+2*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc16_custom, ((bit<16>)meta.tree_id+1)*NUMBER_OF_CELLS+3*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash5 = meta.tree_id_for_hash4 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.csum16, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+4*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash6 = meta.tree_id_for_hash5 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.identity, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+5*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash7 = meta.tree_id_for_hash6 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+6*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash8 = meta.tree_id_for_hash7 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.crc32_custom, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+7*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash9 = meta.tree_id_for_hash8 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.crc16, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+8*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash10 = meta.tree_id_for_hash9 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.crc16_custom, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+9*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash11 = meta.tree_id_for_hash10 + NUMBER_OF_TREES; \
                        hash(register_idx, HashAlgorithm.csum16, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+10*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
