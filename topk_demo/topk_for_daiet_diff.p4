...
#define NUMBER_OF_CELLS 16w167 // to be modifed for adjusting register size
// ----------------------------
// ---------- header ----------
// ----------------------------
...
struct metadata {
    ...
    bit<32> tree_id_for_hash4;
    bit<32> tree_id_for_hash5;
    bit<32> tree_id_for_hash6;
    bit<32> tree_id_for_hash7;
    bit<32> tree_id_for_hash8;
    bit<32> tree_id_for_hash9;
    bit<32> remaining_number_of_entries;
    bit<8>  flush;
...
control MyIngress(inout headers hdr,
    ...
    apply {
        ...
        bit<32> register_idx3;
        bit<32> register_idx4;
        bit<32> register_idx5;
        bit<32> register_idx6;
        bit<32> register_idx7;
        bit<32> register_idx8;
        bit<32> register_idx9;
        bit<KEY_SIZE> stored_key;
        bit<VALUE_SIZE> stored_value;
        bit<VALUE_SIZE> counter_value;
        bit<VALUE_SIZE> counter_value2;
        ...
        if ( hdr.frame_type.frame_type == 0 ) {
            ...
            { \ 
                if(hdr.preamble.remaining_number_of_entries != 0){ \
                    ...
                    pushout_key=0; \
                    pushout_value=0; \
                    hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id-1)*NUMBER_OF_CELLS, { hdr.entry[##i].key }, NUMBER_OF_CELLS); \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx2, HashAlgorithm.crc32_custom, ((bit<16>)meta.tree_id)*NUMBER_OF_CELLS+NUMBER_OF_CELLS, { hdr.entry[##i].key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx2); \
                        topk_value_table_1.read(stored_value, register_idx2); \
                        topk_counter_table_1.read(counter_value2, register_idx2); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx2, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx2, stored_value); \
                            topk_counter_table_1.write(register_idx2, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx2); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx2, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            } \
                        } else {   \
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx2, pushout_key); \
                                topk_value_table_1.write(register_idx2, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx2, counter_value); \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx3, HashAlgorithm.crc16, ((bit<16>)meta.tree_id+1)*NUMBER_OF_CELLS+2*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx3); \
                        topk_value_table_1.read(stored_value, register_idx3); \
                        topk_counter_table_1.read(counter_value2, register_idx3); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx3, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx3, stored_value); \
                            topk_counter_table_1.write(register_idx3, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx3); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx3, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx3, pushout_key); \
                                topk_value_table_1.write(register_idx3, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx3, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash4 = meta.tree_id_for_hash3 + NUMBER_OF_TREES; \
                        hash(register_idx4, HashAlgorithm.crc16_custom, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+3*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx4); \
                        topk_value_table_1.read(stored_value, register_idx4); \
                        topk_counter_table_1.read(counter_value2, register_idx4); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx4, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx4, stored_value); \
                            topk_counter_table_1.write(register_idx4, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx4); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx4, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx4, pushout_key); \
                                topk_value_table_1.write(register_idx4, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx4, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash2 = meta.tree_id + NUMBER_OF_TREES; \
                        hash(register_idx5, HashAlgorithm.csum16, ((bit<16>)meta.tree_id+3)*NUMBER_OF_CELLS+4*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx5); \
                        topk_value_table_1.read(stored_value, register_idx5); \
                        topk_counter_table_1.read(counter_value2, register_idx5); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx5, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx5, stored_value); \
                            topk_counter_table_1.write(register_idx5, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx5); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx5, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx5, pushout_key); \
                                topk_value_table_1.write(register_idx5, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx5, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash2 = meta.tree_id + NUMBER_OF_TREES; \
                        hash(register_idx6, HashAlgorithm.identity, ((bit<16>)meta.tree_id+3)*NUMBER_OF_CELLS+5*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx6); \
                        topk_value_table_1.read(stored_value, register_idx6); \
                        topk_counter_table_1.read(counter_value2, register_idx6); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx6, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx6, stored_value); \
                            topk_counter_table_1.write(register_idx6, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx6); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx6, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx6, pushout_key); \
                                topk_value_table_1.write(register_idx6, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx6, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash2 = meta.tree_id + NUMBER_OF_TREES; \
                        hash(register_idx7, HashAlgorithm.crc32, ((bit<16>)meta.tree_id+3)*NUMBER_OF_CELLS+6*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx7); \
                        topk_value_table_1.read(stored_value, register_idx7); \
                        topk_counter_table_1.read(counter_value2, register_idx7); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx7, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx7, stored_value); \
                            topk_counter_table_1.write(register_idx7, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx7); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx7, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx7, pushout_key); \
                                topk_value_table_1.write(register_idx7, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx7, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash2 = meta.tree_id + NUMBER_OF_TREES; \
                        hash(register_idx8, HashAlgorithm.crc32_custom, ((bit<16>)meta.tree_id+3)*NUMBER_OF_CELLS+7*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx8); \
                        topk_value_table_1.read(stored_value, register_idx8); \
                        topk_counter_table_1.read(counter_value2, register_idx8); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx8, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx8, stored_value); \
                            topk_counter_table_1.write(register_idx8, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx8); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx8, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx8, pushout_key); \
                                topk_value_table_1.write(register_idx8, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx8, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
                    ...
                    if (is_saved != 1) { \
                        meta.tree_id_for_hash2 = meta.tree_id + NUMBER_OF_TREES; \
                        hash(register_idx9, HashAlgorithm.crc16, ((bit<16>)meta.tree_id+3)*NUMBER_OF_CELLS+8*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx9); \
                        topk_value_table_1.read(stored_value, register_idx9); \
                        topk_counter_table_1.read(counter_value2, register_idx9); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                            topk_key_table_1.write(register_idx9, pushout_key); \
                            stored_value = stored_value + pushout_value; \
                            topk_value_table_1.write(register_idx9, stored_value); \
                            topk_counter_table_1.write(register_idx9, counter_value2); \
                            is_saved = 1; \
                            if(stored_key == 0){ \
                                meta.valid_entries_index = meta.valid_entries_index + 1; \
                                valid_entries_stack.write(meta.valid_entries_index, register_idx9); \
                                counter_value2 = counter_value2 + 1; \
                                topk_counter_table_1.write(register_idx9, counter_value2); \
                            } else {  \
                                counter_value2 = counter_value2 + counter_value; \
                            ...
                            if (counter_value2 >= counter_value) { \
                                ...
                                topk_key_table_1.write(register_idx9, pushout_key); \
                                topk_value_table_1.write(register_idx9, pushout_value); \ 
                                pushout_key = stored_key; \
                                pushout_value = stored_value; \
                                topk_counter_table_1.write(register_idx9, counter_value); \
                                counter_value = counter_value2; \
                                is_saved = 2; \
                            } \
                        } \
