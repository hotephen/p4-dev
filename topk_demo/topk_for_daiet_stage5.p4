...
#define NUMBER_OF_CELLS 16w300 // to be modifed for adjusting register size
// ----------------------------
// ---------- header ----------
// ----------------------------
            ...
            { \ 
                if(hdr.preamble.remaining_number_of_entries != 0){ \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id-1)*NUMBER_OF_CELLS+NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id)*NUMBER_OF_CELLS+2*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id+1)*NUMBER_OF_CELLS+3*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
                    ...
                    if (is_saved != 1) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32, ((bit<16>)meta.tree_id+2)*NUMBER_OF_CELLS+4*NUMBER_OF_CELLS, { pushout_key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        topk_counter_table_1.read(counter_value, register_idx); \
                        if (stored_key == 0 || stored_key == pushout_key) {  \
