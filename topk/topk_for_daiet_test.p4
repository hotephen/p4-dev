...
control MyIngress(inout headers hdr,
    ...
    table ipv4_forward {
        ...
        // const entries = {
        //     0x0A000010 : set_egress(2); // 10.0.0.16
        // }
    ...
    action flush_entry##i(bit<5> pushout_table_cnt) { \
        ...
        pushout_key_table.read(hdr.entry[##i].key, (bit<32>)(pushout_table_cnt - 1)); \
        pushout_value_table.read(hdr.entry[##i].value, (bit<32>)(pushout_table_cnt - 1)); \
    ...
    apply {
        ...
        if ( hdr.frame_type.frame_type == 0 ) {
            ...
            { \ 
                if(meta.remaining_number_of_entries != 0){ \
                    ...
                    if (stored_key == 0 || stored_key == hdr.entry[##i].key) { \ 
                        ...
                        if (stored_value >= hdr.entry[##i].value) { \
                    ...
                    if (is_saved == 0) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc32_custom, ((bit<16>)meta.tree_id)*NUMBER_OF_CELLS+1, { hdr.entry[##i].key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        if (stored_key == 0 || stored_key == hdr.entry[##i].key) {  \
                            topk_key_table_1.write(register_idx, hdr.entry[##i].key); \
                    ...
                    if (is_saved == 0) { \
                        ...
                        hash(register_idx, HashAlgorithm.crc16, ((bit<16>)meta.tree_id+1)*NUMBER_OF_CELLS+1, { hdr.entry[##i].key }, NUMBER_OF_CELLS); \
                        topk_key_table_1.read(stored_key, register_idx); \
                        topk_value_table_1.read(stored_value, register_idx); \
                        if (stored_key == 0 || stored_key == hdr.entry[##i].key) {  \
                            topk_key_table_1.write(register_idx, hdr.entry[##i].key); \
            ...
            if ( meta.number_of_entries > 0 ) {
                ...
                hdr.preamble.remaining_number_of_entries = ##i + 1; \
                } \
