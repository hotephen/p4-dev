...
def main():
    ...
    if a.dist == 'z':
        with open('/ssd2/hc/p4-dev/topk/packets/dataset/z_dist_%s_%s' %(p, str(a.entry)), 'r') as f:
            for i in range(a.num): #TODO
                line = f.readline()
                n = line.split()
        ...
        with open('/ssd2/hc/p4-dev/topk/packets/dataset/u_dist_%s' %(str(a.entry)), 'r') as f:
            for i in range(a.num):  #TODO
                line = f.readline()
                n = line.split()
    ...
    for i in range(a.num_flush):
        if a.daiet == 1:
            pkt1 = ether / IP(dst=a.dif, proto=17) / udp / frame_type(frame_type=1, number_of_entries=0) / entry_hdr0(key0=0) # for daiet
        else:
