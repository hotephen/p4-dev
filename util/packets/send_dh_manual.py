...
def main():
    ...
    if len(sys.argv)<7:
        print('[src] [dst] [interface] [vdp_id] [src_mac] [dst_mac] [src_port] [dst_port]')
        exit(1)
    ...
    pkt = DH(vdp_id=vdp_id) / ether / IP(src=src_addr,dst=dst_addr) / TCP(sport=src_port , dport=dst_port)
